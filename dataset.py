...
class Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        intensity_id = self.intensity_map[self.intensity[idx]] if self.load_emotion else None 
        ...
        if self.history_type != "none":
            ...
            if self.history_type == "Guo":
                ...
                for i, h_basename in enumerate(history_basenames):
                    ...
                    h_emotion_id = self.emotion_map[self.emotion[h_idx]]

                    h_intensity_id = self.intensity_map[self.intensity[h_idx]]
                    ...
                    h_mel_path = os.path.join(
                        self.preprocessed_path,
                        "mel_{}".format(self.pitch_level_tag),
                        "{}-mel-{}.npy".format(h_speaker, h_basename),
                    )
                    h_mel = np.load(h_mel_path)

                    history_text_emb.append(h_text_emb)
                    history_speaker.append(h_speaker_id)
                    history_emotion.append(h_emotion_id)
                    history_intensity.append(h_intensity_id)
                    history_mel.append(h_mel)
    ...
    def pad_history(self,
        ...
        for _ in range(pad_size):
            ...
            history_text_emb.append(
                np.zeros(self.text_emb_size, dtype=np.float32)) if history_text_emb is not None else None
            history_text_len.append(
            ...
            history_intensity.append(0) if history_intensity is not None else None
            history_speaker.append(
                0) if history_speaker is not None else None  # meaningless zero padding, should be cut out by mask of history_len
            history_mel_len.append(
    ...
    def reprocess(self, data, idxs):
        ...
        if self.history_type != "none":
            if self.history_type == "Guo":
                ...
                history_emotions = [data[idx]["history"]["history_emotion"] for idx in idxs]
                history_intensitys = [data[idx]["history"]["history_intensity"] for idx in idxs]

                history_mels = [data[idx]["history"]["history_mel"] for idx in idxs]
                ...
                history_emotions = np.array(history_emotions)
                history_intensitys = np.array(history_intensitys)
...
class TextDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        speaker_id = self.speaker_map[speaker]
        emotion_id = self.emotion_map[self.emotion[idx]] if self.load_emotion else None
        intensity_id = self.intensity_map[self.intensity[idx]] if self.load_emotion else None  # 载入情感就把情感强度也加上
        raw_text = self.raw_text[idx]
        phone = np.array(text_to_sequence(self.text[idx], self.cleaners))
        spker_embed = np.load(os.path.join(
            self.preprocessed_path,
    ...
    def collate_fn(self, data):
        ...
        intensitys = np.array([d[6] for d in data]) if self.load_emotion else None
        texts = pad_1D(texts)
        ...
        if self.history_type != "none":
            if self.history_type == "Guo":
                ...
                history_emotions = [d[6]["history_emotion"] for d in data]
                history_mels = [d[6]["history_mel"] for d in data]
                history_intensitys = [d[6]["history_intensity"] for d in data]

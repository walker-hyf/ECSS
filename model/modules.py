...
class VarianceAdaptor(nn.Module):
    ...
    def __init__(self, preprocess_config, model_config, train_config):
        ...
        self.intensity_embedding_layer = nn.Embedding(num_embeddings=3, embedding_dim=256)
        self.emotion_embedding_layer = nn.Embedding(num_embeddings=7, embedding_dim=256)


        self.emotion_weight = nn.Parameter(torch.Tensor([1.0]))    # emotion_emdedding
        self.speaker_weight = nn.Parameter(torch.Tensor([1.0]))    # speaker_embedding
        self.style_weight = nn.Parameter(torch.Tensor([1.0]))      # style_embedding
        self.intensity_weight = nn.Parameter(torch.Tensor([1.0]))  # intensity_emdedding
    ...
    def forward(
        ...
        intensity_emdedding = None
        # 情感强度
        if intensity is not None:
            for inten in intensity:
                is_zero = torch.all(inten == 0)
                if (is_zero):
                    # 如果情感强度为空
                    if (intensity_emdedding == None):
                        intensity_emdedding = torch.zeros(1, 1, 256).to(intensity.device)
                    else:
                        intensity_emdedding = torch.cat(
                            [intensity_emdedding, torch.zeros(1, 1, 256).to(intensity.device)], 0)
                else:
                    # 有情感强度
                    i_max_value, i_max_index = torch.max(inten, dim=0)
                    if (intensity_emdedding == None):
                        intensity_emdedding = self.intensity_embedding_layer(i_max_index).unsqueeze(0).unsqueeze(0)
                    else:
                        intensity_emdedding = torch.cat([intensity_emdedding,
                                                         self.intensity_embedding_layer(i_max_index).unsqueeze(
                                                             0).unsqueeze(0)], 0)

            x = x + intensity_emdedding.expand(
                -1, text.shape[1], -1
            ) * self.intensity_weight

        emotion_emdedding = None

        if emotion is not None:
            for emo in emotion:
                is_zero = torch.all(emo == 0)
                if (is_zero):

                    if (emotion_emdedding == None):
                        emotion_emdedding = torch.zeros(1, 1, 256).to(emotion.device)
                    else:
                        emotion_emdedding = torch.cat([emotion_emdedding, torch.zeros(1, 1, 256).to(emotion.device)], 0)
                else:

                    i_max_value, i_max_index = torch.max(emo, dim=0)
                    if (emotion_emdedding == None):
                        emotion_emdedding = self.emotion_embedding_layer(i_max_index).unsqueeze(0).unsqueeze(0)
                    else:
                        emotion_emdedding = torch.cat(
                            [emotion_emdedding, self.emotion_embedding_layer(i_max_index).unsqueeze(0).unsqueeze(0)], 0)

            x = x + emotion_emdedding.expand(
                -1, text.shape[1], -1
            ) * self.emotion_weight
...
class ProsodyExtractor(nn.Module):
    ...
    def __init__(self, n_mel_channels, d_model, out_dim, kernel_size):
        ...
        self.linear_one = nn.Linear(2 * d_model, 2 * d_model - 128)
        self.linear_two = nn.Linear(2 * d_model - 128, out_dim)
    ...
    def forward(self, mel):
        ...
        out = self.linear_one(h_n)
        out = self.linear_two(out)
        return out


class stylePredictor(nn.Module):
    """ 当前语句的韵律风格提取器（使用Multi-Head Attention） """
...
class emotionalPredictor(nn.Module):
    ...
    def __init__(self, d_model, kernel_size, hidden_size):
        ...
        # 全连接层
        self.liner_one = nn.Linear(hidden_size * 2, 256)  # 512 -> 256
        self.liner_two = nn.Linear(256, 7)  # 256 -> 7

    def forward(self, input):
        output = self.conv_stack(input).squeeze(-1)  # [1,5,384]
        output, (hn, cn) = self.lstm(output)  # [1,5,512]

        hn = torch.cat([hn[0], hn[1]], dim=1)  # 合并LSTM的最后一个时间步的隐层结果(正向+反向)
        hn = self.liner_one(hn)
        hn = self.liner_two(hn)
        return hn
...
class intensityPredictor(nn.Module):
    ...
    def __init__(self, d_model, kernel_size, hidden_size):
        ...
        # 全连接层
        self.liner_one = nn.Linear(hidden_size, 128)
        self.liner_two = nn.Linear(128, 6)
    ...
    def forward(self, input):
        output = self.conv_stack(input).squeeze(-1)  # [1,5,384]
        output, (hn, cn) = self.lstm(output)  # [1,5,256]
        hn = hn.permute(1, 0, 2)  # 调整次序(同时使用正向和反向的值)
        hn = self.liner_one(hn)
        hn = self.liner_two(hn)
        hn = self.avgpool(hn).squeeze(0)
        return hn

# 情感预测器
# emotionalPredictor = emotionalPredictor(d_model=384,kernel_size=3,hidden_size=256)
# input = torch.randn(1,5,384).unsqueeze(-1)
# output = emotionalPredictor(input)  # 两个卷积层+双向LSTM+全连接层 [1,256]
#
# print(output)

    ...
    if len(data) == 17:
        ...
        if history_info is not None:
            if len(history_info) == 7: # "Guo"
                ...
                history_emotions = torch.from_numpy(history_emotions).long().to(device)
                history_intensitys = torch.from_numpy(history_intensitys).long().to(device)
    ...
    if len(data) == 9:
        ...
        if history_info is not None:
            if len(history_info) == 7: # "Guo"
                ...
                history_intensitys = torch.from_numpy(history_intensitys).long().to(device)
                # 待补充mel
                history_mels = torch.tensor(history_mels).to(device)
...
def log(
    ...
    if losses is not None:
        ...
        logger.add_scalar("Loss/style_loss", losses[8], step)
        logger.add_scalar("Loss/intensity_loss", losses[9], step)
        logger.add_scalar("Loss/emotion_loss", losses[10], step)

...
class Preprocessor:
    def __init__(self, preprocess_config, model_config, train_config):
        ...
        self.val_dialog_ids_prior_frame = self.get_val_dialog_ids_prior(os.path.join(self.out_dir, "val_frame_yuan.txt"))
        self.val_dialog_ids_prior_phone = self.get_val_dialog_ids_prior(os.path.join(self.out_dir, "val_phone_yuan.txt"))
    ...
    def build_from_path(self):
        ...
        with open(os.path.join(self.out_dir, "filtered_out_dialog_phone.txt"), "w", encoding="utf-8") as f:
            for m in sorted(filtered_out_dialog_phone):
                f.write(str(m) + "\n")
        with open(os.path.join(self.out_dir, "train_frame_yuan.txt"), "w", encoding="utf-8") as f:
            for m in train_frame:
                f.write(m + "\n")
        with open(os.path.join(self.out_dir, "val_frame_yuan.txt"), "w", encoding="utf-8") as f:
            for m in val_frame:
                f.write(m + "\n")
        with open(os.path.join(self.out_dir, "train_phone_yuan.txt"), "w", encoding="utf-8") as f:
            for m in train_phone:
                f.write(m + "\n")
        with open(os.path.join(self.out_dir, "val_phone_yuan.txt"), "w", encoding="utf-8") as f:
            for m in val_phone:
                f.write(m + "\n")
